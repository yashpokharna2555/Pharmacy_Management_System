...
if (isset($_POST['tuma'])) {
    ...
    $t = time();  // Correct usage of time() without any arguments
...
$getCid = $mysqli->query("SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ");
$getName = $mysqli->query("SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ");
$details1 = $getName->fetch_assoc();
$details = $getCid->fetch_assoc();

$sqlP = $mysqli->query("INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
    ...
    public function ColoredTable($header, $data) {
        ...
        foreach ($data as $row) {
            ...
            if (is_numeric($row[1])) {
                ...
                // Handle the case where row[1] is not a numerical value
                // For example, you might display an empty cell or a placeholder:
                $this->Cell($w[1], 4, '', 'LR', 0, 'R', $fill);
            }
...
$pdf->SetDefaultMonospacedFont('courier'); // Use a valid monospaced font
